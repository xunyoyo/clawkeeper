...
export async function scanSkill(input, options = {}) {
  ...
  for (const file of files) {
    ...
    for (const pattern of rules.skillRiskPatterns ?? []) {
      if (pattern.target !== target) continue;
      if (!matches(content, pattern)) continue;
...
async function loadRules(customRulesPath) {
  const rulesPath = customRulesPath
    ...
    : path.join(path.dirname(new URL(import.meta.url).pathname), '..', '..', 'skill', 'configs', 'core-rules.json');
...
function classifyTarget(relativePath) {
  if (relativePath.endsWith('.sh') || relativePath.endsWith('.bash')) return 'script';
  if (relativePath === 'SKILL.md' || relativePath.endsWith('.md')) return 'skill';
  return 'other';
}
...
function evaluateSkillName(skillName, rules) {
  ...
  if (/(clawh|skilkeeper|openclaww|officia1)/i.test(normalized)) {
    findings.push({
      ...
      file: '(name)',
      evidence: { skillName },
      remediation: 'Verify publisher, repository source, and installation source. Avoid installing skills with similar naming.',
      canAutoFix: false,
      nextStep: `Confirm that ${skillName} is the expected skill. If the source is unclear, do not install it yet.`
    });
  }

  if ((rules.suspiciousSkillNames ?? []).some((token) => nameParts.includes(token))) {
    findings.push({
      id: 'name.high-lure-theme',
      severity: 'LOW',
      title: 'Skill name uses high-lure theme keywords',
      file: '(name)',
      evidence: { skillName, matchedTheme: (rules.suspiciousSkillNames ?? []).find((token) => nameParts.includes(token)) },
      remediation: 'Increase source verification for skills containing keywords like updater, wallet, installer.',
      canAutoFix: false,
...
function scanPrerequisites(content, relativePath, rules) {
  ...
  for (const phrase of rules.dangerousPrerequisitePatterns ?? []) {
    if (!lower.includes(String(phrase).toLowerCase())) continue;
    findings.push({
      ...
      file: relativePath,
      evidence: { phrase },
      remediation: 'Change prerequisite steps to minimal privilege installation flow without requiring system protection to be disabled.',
      canAutoFix: false,
...
async function checkRequiredFiles(skillDir) {
  const findings = [];
  const required = [
    { file: 'SKILL.md', severity: 'MEDIUM', title: 'Missing skill main rules file', remediation: 'Add SKILL.md with clear execution boundaries.' },
    { file: 'skill.json', severity: 'LOW', title: 'Missing skill metadata file', remediation: 'Add skill.json with name, version, and entry point.' }
  ];
...
function buildNextSteps(findings) {
  if (findings.length === 0) {
    return ['No known dangerous patterns detected. You can proceed with manual review of critical side-effect scripts.'];
  }

  const highRisk = findings.filter((item) => item.severity === 'CRITICAL' || item.severity === 'HIGH');
  const steps = [];
  if (highRisk.length > 0) {
    steps.push(`Address high-risk items first: ${highRisk.map((item) => `${item.id}@${item.file}`).join(', ')}.`);
  }
