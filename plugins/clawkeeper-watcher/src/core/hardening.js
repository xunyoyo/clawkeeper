...
export async function harden(stateDir, pluginConfig = {}) {
  ...
  await backupFile(configPath, path.join(backupDir, path.basename(configPath)), files, path.basename(configPath));
  await backupFile(path.join(stateDir, 'AGENTS.md'), path.join(backupDir, 'AGENTS.md'), files, 'AGENTS.md');

  for (const control of getControls()) {
    if (!control.remediate) continue;
    const outcome = await control.describe(context);
    if (!outcome?.autoFixable) continue;
