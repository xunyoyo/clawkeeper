...
async function resolveWorkspaceDir() {
  ...
  if (debugLogger) {
    debugLogger.debug('[Clawkeeper-Watcher Logger] Resolving workspace from candidates:', candidates);
  }
...
async function getTodayLogFile() {
  try {
    ...
    if (debugLogger) {
      debugLogger.error('[Clawkeeper-Watcher Logger] getTodayLogFile error:', error.message, error.stack);
    }
    throw error;
  }
}
...
async function logEvent(eventType, eventData = {}) {
  try {
    ...
    if (debugLogger) {
      debugLogger.error(`[Clawkeeper-Watcher Logger] ✗ Logging error for ${eventType}:`, error.message);
    }
  }
}
...
export function createToolLoggerHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      ...
      if (debugLogger) {
        debugLogger.debug('[Clawkeeper-Watcher Logger] Hook triggered: before_tool_call', { toolName });
      }
      ...
      if (debugLogger) {
        debugLogger.error('[Clawkeeper-Watcher Logger] ✗ before_tool_call hook failed:', error.message);
      }
    }
...
export function createMessageReceivedHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      ...
      if (debugLogger) {
        debugLogger.error('[Clawkeeper-Watcher Logger] ✗ message_received hook failed:', error.message);
      }
    }
...
export function createMessageSendingHook(logger = null) {
  ...
  return async (event, ctx) => {
    try {
      ...
      if (debugLogger) {
        debugLogger.error('[Clawkeeper-Watcher Logger] ✗ message_sending hook failed:', error.message);
      }
    }
    ...
    try {
      ...
      if (Array.isArray(event.assistantTexts)) {
        assistantTexts = event.assistantTexts.map(text => 
          text ? text.substring(0, 2000) : ''  // 截断至2000字符保留完整内容
        );
